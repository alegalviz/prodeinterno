...
Entrust::routeNeedsPermission( 'admin/prodes*', 'manage_roles', Redirect::to('/admin') );
...
{
    $token = Request::ajax() ? Request::header('X-CSRF-Token') : Input::get('csrf_token');
    if (is_null($token)) $token = Input::get('_token');
    if (is_null($token)) $token = Input::get('csrf_token');
